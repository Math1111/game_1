...
while running:
    ...
    for i, (missile, missile_speed_x, missile_speed_y) in enumerate(missiles):
        if ship_alive and missile.colliderect(ship):
            ...
            print(f"Попадание! Очки: {score}")  # Выводим сообщение о попадании

    # Проверка на конец игры (если очки равны 0)
    if score <= 0:
        background_color = GAME_OVER_COLOR  # меняем фон на черный
        print("Игра окончена!")
